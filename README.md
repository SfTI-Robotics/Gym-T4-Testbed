...
```
usage: train_cvae.py [-h] [--N N] [--new_model] [--epochs EPOCHS]
...
optional arguments:
  ...
  --method METHOD      type of rollout trained or random
```
...
### DQN Agent
To run scripts for the DQN agent, first run `cd agents/model_based/agents/dqn_agent` from the root directory.
...
```
usage: train_dqn.py [-h] [--env_name ENV_NAME] [--new_model]
...
### Next State Agent
To run scripts for the DQN agent, first run `cd agents/model_based/agents/next_agent` from the root directory.

#### Generating Rollout Data
Prior to generating the rollout data for this agent, ensure that a predictive model and a DQN agent is already trained for this environment.
...
optional arguments:
  ...
  --resolution RESOLUTION
                        step resolution for performance graph e.g. 100 = time
                        for 100 steps
```
